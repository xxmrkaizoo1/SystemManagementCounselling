                ...
                <aside id="history-sidebar"
                    ...
                    <nav class="space-y-3 text-sm">
                        <a href="{{ route('inbox') }}" title="Inbox" aria-label="Inbox"
                            class="relative flex w-full items-center gap-3 rounded-xl border border-slate-200/80 bg-white/95 px-3 py-2.5 text-slate-600 hover:border-sky-200 hover:text-sky-700 transition">
                            ...
                            <form method="GET" action="{{ route('booking.history') }}"
                                ...
                                <div class="w-full sm:w-auto">
                                    ...
                                    <select id="status" name="status"
                                        class="w-full rounded-lg border-slate-300 text-sm focus:border-sky-400 focus:ring-sky-200">
                                ...
                                <button type="submit"
                                    class="w-full sm:w-auto rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-semibold text-sky-700 hover:bg-sky-100 transition">Filter</button>
                            </form>

                            <div class="hidden sm:block overflow-x-auto rounded-2xl border border-slate-200 bg-white">
                                    ...
                                    <article class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                                        <div class="flex items-start justify-between gap-2">
                                            <div>
                                                ...
                                                <p class="text-sm font-semibold text-slate-800">{{ $booking['date'] }}
                                                </p>
                                            </div>
                                            <span
                                                class="inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold {{ $booking['status_badge_class'] }}">{{ $booking['status_label'] }}</span>
                                        </div>
                                        <div class="mt-2 grid grid-cols-2 gap-2 text-xs">
                                            <div>
                                                <p class="text-slate-500">Time</p>
                                                <p class="font-medium text-slate-700">{{ $booking['time'] }}</p>
                                            </div>
                                            <div>
                                                <p class="text-slate-500">Counsellor</p>
                                                <p class="font-medium text-slate-800">{{ $booking['counsellor'] }}</p>
                                            </div>
                                        </div>
                                        <div class="mt-2">
                                            <p class="text-slate-500 text-xs">Topic / Note</p>
                                            <p class="text-sm text-slate-700 leading-relaxed">{{ $booking['note'] }}
                                            </p>
                                        </div>
                                    </article>
                                @empty
