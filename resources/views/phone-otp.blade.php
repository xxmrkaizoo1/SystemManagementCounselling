...
<body class="min-h-screen bg-slate-50 text-slate-700 overflow-x-hidden">
    ...
    <main class="min-h-screen p-4 sm:p-8 grid place-items-center">
        <section
            class="w-full max-w-md rounded-3xl border border-slate-200/80 bg-white/90 shadow-2xl p-6 sm:p-8 space-y-4">
            <div>
            ...
            <form method="POST" action="{{ route('phone.otp.verify') }}" class="space-y-3">
                @csrf
                <label for="otp" class="block text-sm font-medium text-slate-700">Telegram OTP</label> <input
                ...
                <button type="submit"
                    class="w-full rounded-xl bg-sky-600 text-white font-semibold py-2.5 hover:bg-sky-700 transition shadow-sm">Verify
                    OTP</button>
            </form>
