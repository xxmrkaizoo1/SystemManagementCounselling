...
<body class="min-h-screen bg-slate-50 text-slate-700 overflow-x-hidden">
    ...
    <main class="min-h-screen p-4 sm:p-8">
        <section class="max-w-[96rem] mx-auto rounded-[2rem] border border-slate-200/80 bg-white/80 backdrop-blur-xl shadow-2xl overflow-hidden">
            <header class="px-5 sm:px-7 py-4 border-b border-slate-200/80 bg-white/85 flex items-center justify-between gap-4">
                <div>
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('home') }}" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 hover:text-sky-700 hover:border-sky-200 transition">Home</a>
            ...
            <div class="p-5 sm:p-7 grid xl:grid-cols-[230px_1fr] gap-5">
                <aside class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="flex items-center gap-3 mb-4 pb-3 border-b border-slate-200">
                        <img src="{{ $user->profile_pic ?: '/images/default-profile.svg' }}" alt="Profile" class="w-11 h-11 rounded-full border border-slate-200 object-cover bg-sky-50" />
                        <div>
                    ...
                    <nav class="space-y-2 text-sm">
                        <a href="#overview" class="block rounded-xl border border-slate-200 bg-white px-3 py-2 hover:border-sky-200 hover:text-sky-700 transition">System overview</a>
                        <a href="#roles" class="block rounded-xl border border-slate-200 bg-white px-3 py-2 hover:border-sky-200 hover:text-sky-700 transition">Role distribution</a>
                        <a href="#users" class="block rounded-xl border border-slate-200 bg-white px-3 py-2 hover:border-sky-200 hover:text-sky-700 transition">Recent users</a>
                        <a href="#notifications" class="block rounded-xl border border-slate-200 bg-white px-3 py-2 hover:border-sky-200 hover:text-sky-700 transition">Recent inbox</a>
                    </nav>
                </aside>
                                    ...
                                    <tbody>
                                        @forelse ($recentUsers as $recentUser)
                                            <tr class="border-b border-slate-100">
                                                <td class="py-2 pr-2 font-medium text-slate-700">{{ $recentUser->name }}</td>
                                                <td class="py-2 pr-2 text-slate-600">{{ $recentUser->email }}</td>
                                                <td class="py-2 text-slate-500">{{ optional($recentUser->created_at)->diffForHumans() }}</td>
                                            </tr>
                                        @empty
                                            <tr>
                        ...
                        <div class="mt-4 grid md:grid-cols-2 xl:grid-cols-3 gap-3">
                            @forelse ($recentNotifications as $notification)
                                <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                    <p class="text-xs text-slate-500">{{ optional($notification->created_at)->diffForHumans() }}</p>
                                    <p class="font-semibold text-slate-700 mt-1">{{ $notification->title ?: 'Notification' }}</p>
                                    <p class="text-sm text-slate-600 mt-1">{{ $notification->message }}</p>
                                    <p class="text-xs text-slate-500 mt-2">To: {{ $notification->user?->name ?: 'Unknown user' }}</p>
                                </div>
                            @empty
