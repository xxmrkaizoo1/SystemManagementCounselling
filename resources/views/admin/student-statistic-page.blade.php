...
<head>
    ...
    <style>
        ...
        .animate-modal-overlay {
            animation: modal-fade-in .22s ease-out both;
        }

        .animate-modal-card {
            animation: modal-pop-in .28s cubic-bezier(.2, .85, .25, 1) both;
        }
    </style>
</head>
...
<body class="min-h-screen bg-slate-100 text-slate-700 overflow-x-hidden">
    ...
    <main class="min-h-screen p-2 sm:p-6 lg:p-8">
            ...
            <div class="p-4 sm:p-6 lg:p-7">
                <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    ...
                    <form method="GET" action="{{ route('admin.student-statistics') }}"
                        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 w-full lg:w-auto">
        ...
        <div id="profile-modal"
            class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/70 p-2 sm:p-4">
            <div id="profile-modal-card"
                class="w-full max-w-lg max-h-[92vh] overflow-y-auto overflow-x-hidden rounded-3xl bg-white shadow-2xl border border-rose-100">
                <div class="h-24 bg-gradient-to-r from-indigo-100 via-sky-100 to-rose-100"></div>
                <div class="px-4 sm:px-5 pb-5 -mt-12">
                    <div class="flex items-start justify-end">
                        <button type="button" id="profile-modal-close"
                            class="rounded-lg border border-rose-100 bg-white px-2.5 py-1 text-sm text-rose-500 hover:bg-rose-50">✕</button>
                    </div>
                    <div class="-mt-8 flex flex-col items-center text-center">
                        ...
                        <p id="profile-modal-role" class="text-sm text-indigo-500 capitalize font-medium">-</p>
                    </div>
                    <div class="mt-4 grid grid-cols-2 gap-2 text-xs sm:text-sm">
                        <div class="rounded-xl bg-amber-50 border border-amber-100 px-3 py-2"><span
                                class="text-amber-700">Status</span>
                            <p id="profile-modal-status" class="font-semibold text-amber-900 capitalize">-</p>
                        </div>
                        <div class="rounded-xl bg-sky-50 border border-sky-100 px-3 py-2"><span
                                class="text-sky-700">Phone</span>
                            <p id="profile-modal-phone" class="font-semibold text-sky-900">-</p>
                        </div>
                    </div>
                    <dl class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div class="rounded-xl bg-slate-50/90 border border-slate-100 px-3 py-2">
                            <dt class="text-slate-500">Email</dt>
                            <dd id="profile-modal-email" class="font-medium text-slate-800 break-words">-</dd>
                        </div>
                        <div class="rounded-xl bg-slate-50/90 border border-slate-100 px-3 py-2">
    ...
    <script>
        ...
        const closeProfileModal = () => {
            ...
            profileModal?.classList.remove('animate-modal-overlay');
            document.getElementById('profile-modal-card')?.classList.remove('animate-modal-card');
        };

        document.querySelectorAll('[data-profile-open]').forEach((button) => {
            button.addEventListener('click', () => {
                document.getElementById('profile-modal-name').textContent = prettyValue(button.dataset
                    .name);
                document.getElementById('profile-modal-email').textContent = prettyValue(button.dataset
                    .email);
                document.getElementById('profile-modal-role').textContent = prettyRole(button.dataset.role);
                document.getElementById('profile-modal-phone').textContent = prettyValue(button.dataset
                    .phone);
                document.getElementById('profile-modal-matriks').textContent = prettyValue(button.dataset
                    .matriks);
                document.getElementById('profile-modal-programme').textContent = prettyValue(button.dataset
                    .programme);
                document.getElementById('profile-modal-years').textContent = prettyValue(button.dataset
                    .years);
                document.getElementById('profile-modal-status').textContent = prettyValue(button.dataset
                    .status);
                document.getElementById('profile-modal-pic').src = button.dataset.pic ||
                    '/images/default-profile.svg';

                profileModal?.classList.remove('hidden');
                profileModal?.classList.add('flex');
                profileModal?.classList.add('animate-modal-overlay');
                document.getElementById('profile-modal-card')?.classList.remove('animate-modal-card');
                requestAnimationFrame(() => {
                    document.getElementById('profile-modal-card')?.classList.add(
                        'animate-modal-card');
                });
            });
        });
