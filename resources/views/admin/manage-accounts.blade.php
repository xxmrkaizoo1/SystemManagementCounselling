        ...
        <section
            ...
            <header
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Kelola akaun pelajar, pensyarah, dan kaunselor di satu
                        halaman.</p>
                </div>
            ...
            <div class="p-6 sm:p-7">
                <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"
                    aria-label="Recent user accounts">
                    ...
                    <div class="mt-4 overflow-auto">
                        <table class="w-full text-sm">
                            <caption class="sr-only">Latest managed user accounts</caption>
                            <thead>
                            ...
                            <tbody id="managed_users_table">
                                @forelse ($managedUsers as $managedUser)
                                    <tr class="border-b border-slate-100 managed-user-row"
                                        ...
                                        <td class="py-2 pr-2 font-medium text-slate-700">{{ $managedUser->name }}</td>
                                        <td class="py-2 pr-2 text-sky-700 capitalize">{{ $managedUser->role_name }}</td>
                                        <td class="py-2 pr-2 text-slate-600">{{ $managedUser->email }}</td>
                                        <td class="py-2 text-slate-500">{{ $managedUser->phone ?: '—' }}</td>
                                    </tr>
                                @empty
                                    <tr id="no-data-row">
                                        <td class="py-2 text-slate-500" colspan="4">Tiada data akaun ditemui.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
